...
def stock_data(ticker, period='4y', interval='1d', output_dir='.', write_csv=True):
    ...
    if write_csv:
        ...
        print(f"Saved stock data CSV: {csv_filename}")
...
# Example usage:
# results = lstm_predict_with_sentiment('tsla_matched_with_sentiment.csv')
...
def main_pipeline(ticker, period='4y', interval='1d',
                  ...
                  time_step=60, epochs=100, batch_size=32):
    import os
    import pandas as pd
    ...
    news_df = fetch_gdelt_news_batch(news_query, start_year, end_year, total_records=total_news_records)
    news_csv = f'{ticker}_news_{start_year}_{end_year}.csv'
    news_df.to_csv(news_csv, index=False)
    print(f"News data saved to {news_csv}")
    ...
    # 6. Run LSTM prediction and plot
    results = lstm_predict_with_sentiment(merged_csv, time_step=time_step, epochs=epochs, batch_size=batch_size, plot=True)
